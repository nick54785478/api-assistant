...
public class ChatApplicationService {
     ...
     */
    public ChatApplicationService(ChatClient.Builder chatClientBuilder, List<ToolCallbackProvider> toolProviders, AgentSessionRepositoryPort agentSessionRepositoryPort, PlaybookRepositoryPort playbookRepositoryPort, SessionToolProvider sessionToolProvider, ChatStrategyRegistryPort strategyRegistry) {
        this.agentSessionRepositoryPort = agentSessionRepositoryPort;
        this.playbookRepositoryPort = playbookRepositoryPort;
        this.sessionToolProvider = sessionToolProvider;
        ...
        try {
            ...
            if (selectedStrategy != null && data.session() != null) {
                ...
                if (result == ChatStrategy.PreProcessResult.SAVE_AND_CLEAR) {
                    ...
                    if (data.session().getPlaybookId() != null) {
                        userMessage = "好的，請繼續執行當前步驟。若具備所有參數請直接呼叫工具，若缺少參數請向我詢問。" + contextText;
                    }
                }
                
                if (result == ChatStrategy.PreProcessResult.SAVE_SESSION && data.session().getPlaybookId() != null) {
                    userMessage = "好的，請繼續執行當前步驟。若具備所有參數請直接呼叫工具，若缺少參數請向我詢問。";
                }
            }
            ...
            if (!activeCallbacks.isEmpty() && looksLikeFailedToolCall(result)) {
                log.warn("Detected failed tool call in non-streaming chat for session {}. Retrying...", sessionId);
                
                var retryPrompt = chatClient.prompt()
                    .user("你剛才把工具呼叫輸出為純文字了，這是錯誤的。請使用 Tool Calling 機制（function calling）來呼叫工具。請立刻重新嘗試。")
                    .system(systemPromptText)
                    .advisors(a -> a.param("chat_memory_conversation_id", sessionId))
                ...
                try {
                    ...
                    log.warn("Retry also failed for session {}", sessionId, retryError);
                }
            }
        ...
        try {
            ...
            if (!activeCallbacks.isEmpty()) {
                ...
                try {
                    ...
                    // 偵測 AI 是否輸出了「偽工具呼叫」的純文字
                    if (looksLikeFailedToolCall(result)) {
                        log.warn("Detected failed tool call in AI response for session {}. Response: {}", sessionId, result.substring(0, Math.min(200, result.length())));
                        ...
                        try {
                            String retryResult = retryPrompt.call().content();
                            if (retryResult != null && !retryResult.isBlank() && !looksLikeFailedToolCall(retryResult)) {
                                log.info("Retry succeeded for session {}", sessionId);
                                return reactor.core.publisher.Flux.just(retryResult);
                            }
                        } catch (Exception retryError) {
                            log.warn("Retry also failed for session {}", sessionId, retryError);
                        }
                        ...
                        // 重試也失敗，回傳原始結果但附帶提示
                        log.warn("Retry also produced failed tool call for session {}", sessionId);
                    }
    ...
    private ChatRequestData prepareChatRequest(String sessionId) {
        java.util.concurrent.CompletableFuture<java.util.List<String>> toolsFuture = java.util.concurrent.CompletableFuture.supplyAsync(() -> 
            sessionToolProvider.getSessionToolNames(sessionId)
        );
